                        ...
                        apiData.map((el) => {
                                if(keyword.length > 0) {
                                    return <b.Row key={el.id}>
                                        {el.name.toLowerCase().includes(keyword.toLowerCase()) ? <b.Col><Card title={el.name} subtitle={el.type} text="" link="More Detail" img={el.image.medium} idx={el.id}/></b.Col> : null}
                                    </b.Row>
                                } else {
                                    return <b.Row key={el.id}>
                                        <b.Col><Card title={el.name} subtitle={el.type} text="" link="More Detail" img={el.image.medium} idx={el.id}/></b.Col>
                                    </b.Row>
                                }
                            }
                        )
